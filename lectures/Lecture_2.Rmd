...
A continuous response variable, $y$, is assumed to be linearly dependent on one or several predictor variables, $x_1, x_2, \ldots, x_p$

A simple regression model (one predictor) $ y = \beta_0 + \beta_1 x + \epsilon, \epsilon \sim N(0, \sigma^2)$

A multiple regression model (multiple predictors) $y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \cdots+ \beta_p x_p +\epsilon, \epsilon \sim N(0, \sigma^2)$
...
An unbiased estimator for the error variance $\sigma^2$ depends on these distances through $$ \widehat{\sigma}^2 = \sum d_i^2/(n-p-1) = \frac{SSE}{(n-p-1)} = MSE$$
...
plot(Weight ~ Height, data=bodydata,pch = 1)
points(Weight ~ Height, data=bodydata,pch = 16, col = 2, subset = 1:20)
```
...
```{r, echo=FALSE, fig.height=3.5, fig.width=4.5}
plot(Weight ~ Circumference, data=bodydata,pch = 1)
points(Weight ~ Circumference, data=bodydata,pch = 16, col = 2, subset = 1:20)
```
...
So, a measure of explained variability is $$R^2 = \frac{SST-SSE}{SST} = 1-\frac{SSE}{SST}$$
...
$$R^2_{\text{adj}} = 1 - \frac{\widehat\sigma^2}{\widehat{Var}(y)} = 1 - \frac{SSE/(n-p-1)}{SST/(n-1)}$$ The Solution: $R^2_{\text{adjusted}}$ penalizes adding parameters
...
**Test statistic:** $$T_j = \frac{\widehat{\beta}_j-0}{SE(\widehat{\beta}_j)} \sim t_{n-p-1}$$
...
-   Fail to reject $H_0$ if $|T_j| \leq \tau_{\alpha/2, n-p-1}$ (i.e., p-value $\geq 0.05$)
...
-   Age is not significant (p = 0.582 \>\> 0.05)
...
A $(1-\alpha)\cdot 100 \%$ for $\beta_j$ is mathematically given by: $$ \left[ \widehat{\beta}_j\pm \tau_{\alpha/2, (n-p-1)} SE(\widehat{\beta}_j) \right]$$
