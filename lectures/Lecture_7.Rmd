...
-   We have $p$ predictor variables (columns of $\mathbf{X}$).
...
-   This famous (Fisher's or Anderson's) iris data set gives the
    ...
    X2=sepal width, X3=petal length and X4=sepal width, respectively

-   Can we classify to versicolor or virginica based on X-variables?
...
```{r, echo=FALSE}
pairs(iris.train[,-5], col=iris.train[,"Species"], pch=20, cex=2, lower.panel = NULL)
```
...
In discriminant analysis we search for decision regions for the $g$
...
-   The idea is to find decision regions $R_1, R_2, \ldots, R_g$ that
...
$$\color{blue}{\frac{p(c_f|x^*)}{p(c_m|x^*)} = \frac{f_1(x^*)\cdot p_1}{f_2(x^*)\cdot p_2}\geq 1}$$
...
Taking logarithms and simplifying: $$-(x-\mu_1)^2 = -(x-\mu_2)^2$$
$$x^2 - 2x\mu_1 + \mu_1^2 = x^2 - 2x\mu_2 + \mu_2^2$$
$$2x(\mu_2 - \mu_1) = \mu_2^2 - \mu_1^2, \text{ hence }\color{red}{x = \frac{\mu_1 + \mu_2}{2}}$$
...
```{r, echo=TRUE}
DAModel.2 <- lda(Sex~HLS, data=Lizard, prior=c(0.2, 0.8))
...
For two variables $(X_1, X_2)$, the bivariate normal distribution is
characterized by:
...
For lizard example with HLS and Mass ($k=2$):
...
## Why is QDA "Quadratic"? (One covariate)
...
$\frac{p_1}{\sqrt{2\pi\sigma_1^2}}e^{-\frac{(x-\mu_1)^2}{2\sigma_1^2}} = \frac{p_2}{\sqrt{2\pi\sigma_2^2}}e^{-\frac{(x-\mu_2)^2}{2\sigma_2^2}}$Taking
logarithms:
$\log(p_1) - \frac{1}{2}\log(\sigma_1^2) - \frac{(x-\mu_1)^2}{2\sigma_1^2} = \log(p_2) - \frac{1}{2}\log(\sigma_2^2) - \frac{(x-\mu_2)^2}{2\sigma_2^2}$
...
## Visualizing QDA with Different Variances (2D)
...
# Class-specific means (levels: m, f)
mu_m_qda <- qda_biv$means["m", c("HLS", "Mass")]
mu_f_qda <- qda_biv$means["f", c("HLS", "Mass")]
...
# Grid over a larger HLS–Mass space to see curvature better
hls_seq  <- seq(min(Lizard$HLS)  - 20, max(Lizard$HLS)  + 20, length.out = 200)
mass_seq <- seq(min(Lizard$Mass) - 20, max(Lizard$Mass) + 20, length.out = 200)
grid_qda <- expand.grid(HLS = hls_seq, Mass = mass_seq)
X_grid   <- as.matrix(grid_qda[, c("HLS", "Mass")])

# Class scores (proportional to posterior with equal priors)
score_f <- dmvnorm(X_grid, mean = mu_f_qda, sigma = S_f_qda) * 0.5
score_m <- dmvnorm(X_grid, mean = mu_m_qda, sigma = S_m_qda) * 0.5

# Difference for contouring: score_f - score_m = 0 → decision boundary
z_diff <- matrix(score_f - score_m,
                 nrow = length(hls_seq),
                 ncol = length(mass_seq))

# Plot observed data with enlarged axes
plot(Lizard$HLS, Lizard$Mass,
     xlim = c(77,170),
     ylim = c(-10,20),
     col  = ifelse(Lizard$Sex == "f", "red", "blue"),
     pch  = 19, cex = 1.5,
     xlab = "HLS (Hind Limb Span)", ylab = "Mass",
     main = "QDA: Curved Decision Boundary (HLS + Mass)")

# Add curved QDA decision boundary
contour(hls_seq, mass_seq, z_diff, levels = 0,
        add = TRUE, col = "darkgreen", lwd = 3, drawlabels = FALSE)

# Add class means
points(mu_f_qda["HLS"], mu_f_qda["Mass"],
       pch = 4, col = "black",  cex = 3, lwd = 3)
points(mu_m_qda["HLS"], mu_m_qda["Mass"],
       pch = 4, col = "black", cex = 3, lwd = 3)

legend("topleft",
       legend = c("Female", "Male", "QDA boundary", "Class means"),
       col    = c("red", "blue", "darkgreen", "black"),
       pch    = c(19, 19, NA, 4),
       lty    = c(NA, NA, 1, NA),
       lwd    = c(NA, NA, 3, 3),
...
```{r, echo=TRUE}
DAModel.4 <- lda(Sex~SVL, data=Lizard, prior=rep(0.5,2))
