---
title: "STAT 340 Applied Methods in Statistics Course: Lecture 1 - Introduction"
...
-   Advanced mode jumping samplers and other approximation schemes for
    Bayesian inference.
...
2.  **Events** $\mathcal{F}$: set of subsets of outcomes we observe
    -   *Discrete:* $\{\text{H}\}$ = \`\`heads'' or $\{\text{H}\}$
        =\`\`tail''
    -   *Continuous:* $\{h : \text{ between } 160 \text{ and } 180\}$ or
        $\{h : \text{ between } 240 \text{ and } 250\}$
3.  **Probabilities** $P$: assigns to events a number in $[0,1]$
    -   *Discrete:* $P(\text{H}) = 0.5$
    -   *Continuous:* $P(160 \leq h \leq 180) = 0.68$ or
        $P(240 \leq h \leq 250) = 0.001$
...
3.  Countable additivity: If $A_1, A_2, \ldots$ are disjoint events,
...
# Continuous PDF
curve(dnorm(x, 175, 10), xlim=c(120, 250), ylim=c(0, 0.045),
      main='Continuous: PDF', ylab='f(y)', xlab='y',
      lwd=2.5, col='darkred', cex.axis=0.9)


# Continuous CDF
curve(pnorm(x, 175, 10), xlim=c(120, 250), ylim=c(0, 1),
...
(infinity!), so each single point contributes zero probability. Only
**intervals with area** have non-zero probability.
...
-   What % score between **600-700**? (1 to 2 standard deviations away
    from mean)
...
-   What % score between **400-600**? (within 1 standard deviation)
    -   **Answer: 68.2%**
-   What % score between **600-700**? (2 standard deviations away from
    mean)
    -   **Answer: 13.6%**
...
property: $Y = \sum_{i=1}^k Z_i^2 \sim \chi^2_k$
...
**Definition:** For independent $Z \sim N(0,1)$ and $Y\sim\chi^2_k$:

$$T = \frac{Z}{\sqrt{Y / k}} \sim t_k$$

$$
E(T) = 0 \quad \text{ and } \quad  Var(T) = \frac{k}{k-2}
$$
...
![English scientist working as a chemist for Guinness Brewery. Developed
the t distribution to handle small samples when variance is unknown
(1908)](images/clipboard-1638406457.png){alt="English scientist working as a chemist for Guinness Brewery. Developed the t distribution to handle small samples when variance is unknown (1908)"
width="65" height="94"}
...
$$
E(Y) = \frac{v}{v-2}, v>2 \quad Var(Y) = \frac{2v^2(u+v-2)}{u(v-2)^2(v-4)} v>4
$$
...
curve(df(x, df1=5, df2=10), xlim=c(0, 6), ylim=c(0, 1.2), ylab='f(y)', xlab='y',
      main='Right-skewed', lwd=2.5, col='darkred')
curve(df(x, df1=20, df2=20), xlim=c(0, 6), ylim=c(0, 1.2), ylab='f(y)', xlab='y',
      main='More symmetric', lwd=2.5, col='darkgreen', add=TRUE)
...
statistics and evolutionary biology. The founder of modern statistics
and namesake of the
F-distribution.](images/clipboard-465988204.png){width="127"}
...
to a framework for scientific inference.
