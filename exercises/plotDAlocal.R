plotDA <- function (DAobject = NULL, regions = TRUE, contours = FALSE, 
                    resolution = 100) 
{
  if (is.null(DAobject)) {
  ...
  for (i in 1:g) {
    points(the.means[2, i] ~ the.means[1, i], pch = 17, cex = 0.8, 
           col = dark.cols[i])
    points(the.means[2, i] ~ the.means[1, i], pch = 2, cex = 0.8, 
           col = "black")
  }
  if (contours == TRUE) {
    for (i in 1:g) {
