...
E.g. distance between two observations $(x_{11}, x_{21})$ and
$(x_{12}, x_{22})$
...
plot(X1[1], X1[2], pch = 19, col = "red", xlim = c(3, 7), ylim = c(2, 6),
     xlab = "x", ylab = "y", main = "d = sqrt((6.255 - 3.919)^2 + (5.495 - 2.683)^2)")
...
```{r, echo=TRUE}
dist(mypoints, method ="euclidian")
```
...
# Plot
plot(X1[1], X1[2], pch = 19, col = "red", xlim = c(3, 7), ylim = c(2, 6),
     xlab = "x", ylab = "y", main = "manh_dist = abs(X1[1] - X2[1]) + abs(X1[2] - X2[2])")
points(X2[1], X2[2], pch = 19, col = "blue")
...
-   Question: Are there many species of iris? (if we did not know the
    truth)
...
segments(cent1256_x, cent1256_y, x1[4], x2[4], col = 'darkgreen', lwd = 2, lty = 2)
text(x1, x2, labels = 1:6, pos = 3, cex = 1.3, font = 2)
box()
```
...
3.  **Internal validation**: Use quality metrics (e.g., silhouette
    coefficient, within-cluster sum of squares) for different $K$.
...
## Compare some hc and km clusterings
...
-   **Average silhouette**: mean of $s(i)$ over all observations (higher
    ...
    sil <- silhouette(cutree(hc, k = 5), dist(iris[,1:4]))
    mean(sil[, 3]) # Average silhouette width
