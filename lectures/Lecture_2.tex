...
\begin{frame}{The body data}
\phantomsection\label{the-body-data}
A motivating example - The body data

\pandocbounded{\includegraphics[keepaspectratio]{Lecture_2_files/figure-beamer/unnamed-chunk-1-1.pdf}}
\end{frame}
...
\item
  ...
  \(\sum (y_i - (\widehat{\beta}_0 + \widehat{\beta}_1 x_i))^2\)
\item
  Robust estimator : The estimates minimize
  \(\sum |y_i - (\widehat{\beta}_0 + \widehat{\beta}_1 x_i)|\)
\item
  Maximum likelihood estimator: The estimates make the observed data
  ``most likely''
\end{itemize}
\end{frame}
...
\[ \widehat{\sigma}^2 = \sum d_i^2/(n-p) = \frac{SSE}{(n-p)} = MSE\]
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{options}\NormalTok{(}\AttributeTok{contrasts=}\FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}contr.sum\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}contr.poly\textquotesingle{}}\NormalTok{))}
...
\NormalTok{sum.model1 }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(LinearModel}\FloatTok{.1}\NormalTok{)}
\FunctionTok{print}\NormalTok{(sum.model1)}
\end{Highlighting}
\end{Shaded}
\end{frame}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture_2_files/figure-beamer/unnamed-chunk-5-1.pdf}}
\end{frame}
...
\[\sum (y_i - \bar{y})^2/(n-1) = \frac{SST}{(n-1)}\] Hence, \(SST\) (in
some literature \(SSY\)) is a mesure of total variability in y, the
response
...
\begin{frame}[fragile]{The ANOVA table}
\phantomsection\label{the-anova-table}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{Anova}\NormalTok{(LinearModel}\FloatTok{.1}\NormalTok{, }\AttributeTok{type=}\StringTok{"II"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## Height    308.31  1  9.0433 0.007566 **
## Residuals 613.67 18                    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}

The Residuals-line gives the SSE and the total sums of squares, SST,
equals the sum of all elements in the ``Sum sq'' column.
\end{frame}
...
\[R^2_{\text{adj}} = 1 - \frac{MSE}{Var(y)} = 1 - \frac{SSE/(n-p)}{SST/(n-1)}\]
The Solution: \(R^2_{\text{adjusted}}\) penalizes adding parameters

\includegraphics[width=400px]{Lecture_2_files/figure-beamer/unnamed-chunk-10-1}
\end{frame}
...
\[T_j = \frac{\widehat{\beta}_j}{SE(\widehat{\beta}_j)} \sim t_{n-p}\]
\end{frame}

\begin{frame}{Visual explanation}
\phantomsection\label{visual-explanation}
\pandocbounded{\includegraphics[keepaspectratio]{Lecture_2_files/figure-beamer/unnamed-chunk-11-1.pdf}}
\end{frame}
...
\item
  ...
  to observe such an extreme T if \(H_0\) were true → Strong evidence
  against \(H_0\) → \textbf{Reject} \(H_0\) ✓
\end{itemize}
...
\tightlist
\item
  Reject \(H_0\) if \(|T_j| > t_{\alpha/2, n-p}\) (i.e., p-value
  \(< 0.05\))
\item
  Fail to reject \(H_0\) if \(|T_j| \leq t_{\alpha/2, n-p}\) (i.e.,
  p-value \(\geq 0.05\))
\end{itemize}
\end{frame}
...
\begin{itemize}
\item
  Age is not significant (p = 0.260 \textgreater\textgreater{} 0.05)
\item
...
\[ \left[ \widehat{\beta}_j\pm t_{\alpha/2, (n-p)} SE(\widehat{\beta}_j) \right]\]
...
\FunctionTok{predict.lm}\NormalTok{(LinearModel}\FloatTok{.4}\NormalTok{, }\AttributeTok{newdata=}\NormalTok{newobs, }\AttributeTok{level=}\FloatTok{0.95}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=700px,height=400]{Lecture_2_files/figure-beamer/unnamed-chunk-19-1}
\end{frame}
